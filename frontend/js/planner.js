document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadCityData(cityCode) {
        try {
            ...
            if (typeof loadStopData === 'function') {
                ...
                console.timeEnd('站点数据加载');
                console.log(`✅ 站点数据加载完成: ${stopData.length}条`);
            }
            
            // 去重处理（用于最近站点查找）
            if (typeof uniqueStations === 'function') {
                uniqueStopData = uniqueStations(stopDataResult);
                // 调试：检查去重后的站点是否有type属性
                if (uniqueStopData.length > 0) {
                    console.log('去重后站点示例:', uniqueStopData[0]);
                    console.log('站点是否有type属性:', uniqueStopData[0].type);
                }
            } else {
            ...
            console.log(`✅ 核心数据加载完成，站点${stopData.length}条，去重后${uniqueStopData.length}条`);
            
            // 【优化2】POI数据完全延迟加载，不阻塞页面显示
            // 先清空POI数据，页面可以立即使用（仅支持站点搜索）
            currentPOIData = [];
            console.log('⏳ POI数据将在后台加载...');
            
            // 延迟1秒后再加载POI，确保页面已完全显示
            setTimeout(() => {
                loadPOIDataAsync(cityCode);
            }, 1000);
    ...
    // 【优化】POI数据异步加载函数（不阻塞页面）
    async function loadPOIDataAsync(cityCode) {
        try {
            console.log('后台加载POI数据...');
            if (typeof loadPOIData === 'function') {
                const poiData = await loadPOIData(cityCode);
                currentPOIData = poiData;
                console.log(`POI数据后台加载完成: ${poiData.length}条`);
                
                // POI加载完成后，更新自动完成管理器
                if (autocompleteManager && typeof autocompleteManager.updateData === 'function') {
                    autocompleteManager.updateData(currentPOIData, uniqueStopData);
                    console.log('自动完成已更新POI数据，现在可以搜索POI了');
                }
            }
        } catch (error) {
            console.warn('POI数据加载失败（不影响站点搜索）:', error.message);
        }
    }
    ...
    async function setStartSubwayStation(station) {
        ...
        console.log('✅ 设置起点站:', station.name, 'startSubwayStation已设置:', startSubwayStation !== null);
    ...
    async function setEndSubwayStation(station) {
        ...
        console.log('✅ 设置终点站:', station.name, 'endSubwayStation已设置:', endSubwayStation !== null);
    ...
    function initAutocomplete() {
        ...
        // 当输入框内容变化时，检查是否需要清空对应的变量
        // 这样可以避免"输入框有内容但变量为空"的状态不同步问题
        startInput.addEventListener('input', function() {
            if (startSubwayStation && this.value.trim() !== startSubwayStation.name) {
                console.log('起点输入框内容变化，清空起点变量');
                startSubwayStation = null;
            }
        });
        
        endInput.addEventListener('input', function() {
            if (endSubwayStation && this.value.trim() !== endSubwayStation.name) {
                console.log('终点输入框内容变化，清空终点变量');
                endSubwayStation = null;
            }
        });
    ...
    async function handleLocationSelect(item, type) {
        console.log(`选择${type}:`, item.name, '类型:', item.type, '完整item:', item);
        
        // 判断是否为地铁站：type为'地铁站'，或者有lineId/lineName属性
        const isSubwayStation = item.type === '地铁站' || 
                                item.lineId !== undefined || 
                                item.lineName !== undefined ||
                                item.linename !== undefined;
        ...
        if (isSubwayStation) {
            ...
            // 直接选择地铁站
            if (type === 'start') {
                await setStartSubwayStation(stationData);
            } else {
                await setEndSubwayStation(stationData);
            }
        } else {
            // 选择的是POI，查找最近地铁站
    ...
    function initRoutePlanning() {
        ...
        planBtn.addEventListener('click', async function() {
            ...
            console.log('点击规划按钮，当前状态:', {
                startSubwayStation: startSubwayStation ? startSubwayStation.name : null,
                endSubwayStation: endSubwayStation ? endSubwayStation.name : null,
                startInputValue: startInput.value,
                endInputValue: endInput.value
            });
            
            // 如果输入框有内容但变量为空，尝试自动匹配站点
            if (!startSubwayStation && startInput.value.trim()) {
                const matchedStation = uniqueStopData.find(s => s.name === startInput.value.trim());
                if (matchedStation) {
                    console.log('自动匹配起点站:', matchedStation.name);
                    await setStartSubwayStation(matchedStation);
                }
            }
            
            if (!endSubwayStation && endInput.value.trim()) {
                const matchedStation = uniqueStopData.find(s => s.name === endInput.value.trim());
                if (matchedStation) {
                    console.log('自动匹配终点站:', matchedStation.name);
                    await setEndSubwayStation(matchedStation);
                }
            }
            
            if (!startSubwayStation || !endSubwayStation) {
                alert('请先选择起点和终点（请从下拉列表中选择站点）');
                return;
            }
    ...
    function renderRouteOptionsSelector(routes) {
        ...
        routes.forEach((route, index) => {
            ...
            // 生成途经线路标签
            const linesHtml = route.lineNames && route.lineNames.length > 0 
                ? `<div class="option-lines">
                    <span class="lines-label">途经地铁：</span>
                    ${route.lineNames.map(line => `<span class="line-tag">${line}</span>`).join('')}
                   </div>`
                : '';
            
            // 方案内容 - 显示方案编号、类型标签和途经线路
    ...
    function showRouteDetails(route) {
        ...
        if (route.steps && route.steps.length > 0) {
            route.steps.forEach((step, index) => {
                ...
                // 根据步骤类型选择图标
                let iconClass = 'fa-exchange-alt';  // 默认换乘图标
                if (step.type === 'subway') {
                    iconClass = 'fa-subway';
                } else if (step.type === 'walk') {
                    iconClass = 'fa-walking';
                } else if (step.type === 'start') {
                    iconClass = 'fa-sign-in-alt';  // 上车图标
                }
