...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function switchCity(cityCode) {
        showLoading(`正在切换到${cityConfig[cityCode].name}...`);
        ...
        // 加载线路数据
        updateLoadingText(`正在加载${cityConfig[cityCode].name}地铁数据...`);
        loadRailwayData(cityCode)
            .then(railData => {
                ...
                updateLoadingText(`正在渲染${railData.features.length}条线路...`);
    ...
    function loadRailwayData(cityCode) {
        // 使用API加载数据
        return loadMetroLines(cityCode).catch(error => {
    ...
    function showLoading(text = '加载地图数据中...') {
    ...
    function init() {
        ...
        console.log('[初始化] 开始并行加载地图和数据...');
        const startTime = performance.now();

        // 使用 Promise 并行等待：地图加载 + 数据加载
        const mapLoadPromise = new Promise((resolve) => {
            if (map.loaded() && map.isStyleLoaded()) {
                console.log('[初始化] 地图已就绪');
                resolve();
            } else {
                map.once('load', () => {
                    console.log('[初始化] 地图加载完成');
                    resolve();
                });
            }
        });

        const dataLoadPromise = loadRailwayData(currentCity)
            .then(railData => {
                if (!railData || !railData.features || !Array.isArray(railData.features)) {
                    throw new Error('数据格式错误：缺少features数组');
                }
                console.log('[初始化] 数据加载完成，共', railData.features.length, '条线路');
                railwayData = railData;
                return railData;
            });

        // 并行等待两者完成
        Promise.all([mapLoadPromise, dataLoadPromise])
            .then(() => {
                const loadTime = (performance.now() - startTime).toFixed(0);
                console.log(`[初始化] 地图和数据均已就绪，耗时 ${loadTime}ms，开始渲染...`);
                ...
                console.log(`[初始化] 渲染完成，总耗时 ${totalTime}ms`);
            })
            .catch(error => {
