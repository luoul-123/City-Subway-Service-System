...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getURLParams() {
        ...
        return {
            ...
            station: params.get('station')  // 从收藏页面跳转时的站点参数
        };
    }
    ...
    // 8. 加载地铁路线数据和站点数据（使用API）
    function loadRailwayData(cityCode) {
        console.log('[loadRailwayData] 开始加载线路数据, cityCode:', cityCode);
        // 使用API加载数据
        return loadMetroLines(cityCode).then(data => {
            console.log('[loadRailwayData] API加载成功, features数量:', data?.features?.length);
            return data;
        }).catch(error => {
            console.error('[loadRailwayData] API加载失败，尝试本地文件:', error);
            // 降级到本地文件
            const config = cityConfig[cityCode];
            return fetch(config.dataPath)
                ...
                .then(data => {
                    console.log('[loadRailwayData] 本地文件加载成功, features数量:', data?.features?.length);
                    return data;
                });
        });
    }
    function loadStopData(cityCode) {
        console.log('[loadStopData] 开始加载站点数据, cityCode:', cityCode);
        // 使用API加载数据
        return loadMetroStations(cityCode).then(data => {
            console.log('[loadStopData] API加载成功, 数据量:', Object.keys(data?.name || {}).length);
            return data;
        }).catch(error => {
            console.error('[loadStopData] API加载失败，尝试本地文件:', error);
            // 降级到本地文件
            const config = cityConfig[cityCode];
            return fetch(config.stopDataPath)
                ...
                .then(data => {
                    console.log('[loadStopData] 本地文件加载成功, 数据量:', Object.keys(data?.name || {}).length);
                    return data;
                });
        });
    }
    ...
    function initializeLineButtons() {
        console.log('[initializeLineButtons] 开始初始化线路按钮');
        console.log('[initializeLineButtons] railwayData:', railwayData);
        ...
        if (!railwayData || !railwayData.features || railwayData.features.length === 0) {
            console.error('[initializeLineButtons] railwayData为空或features为空!');
            return;
        }

        console.log('[initializeLineButtons] 准备生成', railwayData.features.length, '个线路按钮');
        ...
        const popupContent = `
            ...
            `<span class="transfer-line-badge" style="border-left-color: ${lineColors[line] || getRandomColor(line)}">${line}</span>`
                ...
                <div class="popup-footer">
                    ...
                    <button class="popup-action-btn popup-favorite-btn" id="favorite-station-btn" 
                            data-station="${stationInfo.name}" 
                            data-city="${currentCity}"
                            ${!isLoggedIn ? 'title="请先登录"' : ''}>
                        <i class="fas fa-star" id="favorite-icon"></i> 
    ...
    function bindPopupEvents(stationInfo) {
        ...
        // 收藏按钮事件
        const favoriteBtn = document.getElementById('favorite-station-btn');
        if (favoriteBtn) {
            // 检查当前收藏状态
            checkAndUpdateFavoriteStatus(stationInfo.name, currentCity);
            ...
            favoriteBtn.addEventListener('click', async () => {
                ...
                // 执行收藏/取消收藏操作
                await toggleFavoriteStation(stationInfo.name, currentCity, userId);
            });
        }
    }

    // 检查并更新收藏状态
    async function checkAndUpdateFavoriteStatus(stationName, cityCode) {
        const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
        const userId = localStorage.getItem('userId');
        ...
        if (!isLoggedIn || !userId) {
            ...
            favoriteText.textContent = '登录后收藏';
            return;
        }

        try {
            const response = await fetch(
                `http://127.0.0.1:5000/api/favorite/check?user_id=${userId}&city_code=${cityCode}&station_id=${encodeURIComponent(stationName)}`
            );
    ...
    // 切换收藏状态
    async function toggleFavoriteStation(stationName, cityCode, userId) {
        const favoriteBtn = document.getElementById('favorite-station-btn');
        const favoriteIcon = document.getElementById('favorite-icon');
        const favoriteText = document.getElementById('favorite-text');
        ...
        const apiUrl = isFavorited
            ? 'http://127.0.0.1:5000/api/favorite/remove'
            : 'http://127.0.0.1:5000/api/favorite/add';

        try {
            favoriteBtn.disabled = true;
            favoriteText.textContent = isFavorited ? '取消中...' : '收藏中...';
    ...
    function searchAndShowLine(searchText) {
        ...
        if (matchedLines.length > 0) {
            ...
            matchedLines.forEach((feature, index) => {
                const lineName = feature.properties.name;
                showLine(lineName);
    ...
    function init() {
        ...
        Promise.all([
            ...
            console.log('[初始化] railData:', railData);
            console.log('[初始化] railData.features数量:', railData?.features?.length);

            railwayData = railData;
            stopData = formatStopData(stopDataResult);

            console.log('[初始化] stopData数量:', stopData?.length);
            ...
            function initializeUI() {
                ...
                // 如果URL中有station参数（从收藏页面跳转），自动定位到该站点
                if (urlParams.station) {
                    setTimeout(() => {
                        const stationName = decodeURIComponent(urlParams.station);
                        console.log('[URL参数] 定位到站点:', stationName);

                        const stationInfo = stopData.find(s => s.name === stationName);
                        if (stationInfo) {
                            const lineName = stationInfo.linename;
                            if (lineName && !activeLines.has(lineName)) {
                                const lineButtons = document.querySelectorAll('.line-btn');
                                lineButtons.forEach(btn => {
                                    if (btn.textContent === lineName) {
                                        btn.click();
                            ...
                            setTimeout(() => {
                                ...
                                // 使用 moveend 事件确保飞行动画完全结束后再显示 popup
                                const showPopupAfterFly = () => {
                                    // 等待地图完全稳定后再显示 popup
                                    map.once('idle', () => {
                                        console.log('[URL参数] 地图已稳定，显示站点弹窗');
                                        const isTransferStation = isStationTransfer(stationName);
                                        currentPopupStationInfo = {
                                            name: stationInfo.name,
                                            lineName: stationInfo.linename,
                                            num: stationInfo.num,
                                            coordinates: [lon, lat],
                                            isTransfer: isTransferStation
                                        };
                                        createUnifiedStationPopup(currentPopupStationInfo, [lon, lat]);
                                    });
                                };

                                // 监听飞行动画结束
                                map.once('moveend', showPopupAfterFly);
                            ...
                            console.warn('[URL参数] 未找到站点:', stationName);
                        }
                    }, 800);
                }
            }
            ...
            // 检查地图是否已经加载完成
            // 如果已经加载完成，直接初始化；否则等待load事件
            if (map.loaded()) {
                console.log('[初始化] 地图已加载完成，直接初始化UI');
                initializeUI();
            } else {
                console.log('[初始化] 等待地图加载...');
                map.on('load', () => {
                    console.log('[初始化] 地图加载完成，初始化UI');
                    initializeUI();
                });
            }
        }).catch(error => {
