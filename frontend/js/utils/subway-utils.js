...
window.uniqueStations = function(rawStops) {
    ...
    if (typeof rawStops === 'object' && rawStops.name && !Array.isArray(rawStops)) {
        ...
        stationsArray = indices.map(index => ({
            ...
            linename: rawStops.linename[index] || '',  // 兼容性字段
            ...
            wgsLon: parseFloat(rawStops.lon[index]) || 0,  // 自动完成需要
            wgsLat: parseFloat(rawStops.lat[index]) || 0,  // 自动完成需要
            num: parseInt(rawStops.num[index]) || 0,
            type: '地铁站'  // 添加类型标识，用于区分地铁站和POI
        }));
    } else if (Array.isArray(rawStops)) {
        // 如果已经是数组，确保每个站点都有type属性
        stationsArray = rawStops.map(station => ({
            ...station,
            type: station.type || '地铁站',
            wgsLon: station.wgsLon || station.lon,
            wgsLat: station.wgsLat || station.lat,
