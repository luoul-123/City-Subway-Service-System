class MarkerManager {
    ...
    createRouteMarkers(routeStations, startStation, endStation, intersection) {
        ...
        console.log('传入的换乘站:', intersection);
        ...
        // 支持多个换乘站（可以是字符串或数组）
        let normalizedIntersections = new Set();
        if (intersection) {
            if (Array.isArray(intersection)) {
                // 如果是数组，标准化每个换乘站名
                intersection.forEach(name => {
                    if (name) normalizedIntersections.add(normalizeStationName(name));
                });
            } else {
                // 如果是单个字符串
                normalizedIntersections.add(normalizeStationName(intersection));
            }
        }
        
        console.log('标准化后的换乘站集合:', Array.from(normalizedIntersections));
        ...
        routeStations.forEach((station, index) => {
            ...
            const isTransfer = normalizedIntersections.has(normalizedStationName);
