...
- 来源：高德地图开放平台爬取（参考：https://github.com/YoujiaH/-poi.git）

- 核心文件：'poi_all.json'（包含POI（Point of Interest，兴趣点）信息的JSON文件）

- 字段说明：id：POI 的唯一标识符。name：POI 的名称。type：POI 的分类名称（高德提供的分类标准）。type_code：POI 类型的对应编码。search_type：搜索类型，若为空则表示未指定。lon：POI 的经度（WGS84坐标系）。lat：POI 的纬度（WGS84坐标系）。address：POI 的详细地址。pname：POI 所在的省份名称。cityname：POI 所在的城市名称。adname：POI 所在的区/县名称。tel：POI 的联系电话列表，若无则为空数组 []。business_area：POI 所在的商圈信息，若无则为空数组 []。
