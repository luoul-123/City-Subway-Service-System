...
function highlightCurrentPage() {
    ...
    let pageId = null; // 默认为 null，表示不在导航栏中
    ...
    switch (currentPage) {
        ...
        default: pageId = null; // 不在导航栏中的页面（如 profile.html, login.html 等）不高亮任何项
    }

    const navItems = document.querySelectorAll('.nav-item');
    navItems.forEach(item => {
        // 只有当 pageId 不为 null 且匹配时才添加 active 类
        if (pageId !== null && item.getAttribute('data-page') === pageId) {
...
function initUserLink() {
    const userLink = document.querySelector('.icon-item[title="登录"], .icon-item[title="个人中心"]');
    if (!userLink) return;
    ...
    if (isLoggedIn) {
        ...
        userLink.setAttribute('title', '个人中心');
        // 更新图标（可选，保持用户图标）
        ...
        userLink.setAttribute('title', '登录');
    }
}
